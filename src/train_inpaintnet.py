...
def get_csv_files(split):
    ...
    return [(None, csv_file, None) for csv_file in csv_files]  # Формат: (video_path, csv_path, frame_dir)

def load_coordinate_data(video_path, csv_path, frame_dir, split, seq_len):
    """
    ...
    """
    logger.debug(f"Загрузка данных: csv_path={csv_path}, split={split}, seq_len={seq_len}")
    
    # Конвертация EagerTensor в строки
    if isinstance(csv_path, tf.Tensor):
        csv_path = csv_path.numpy().decode('utf-8')
    ...
    if not all(col in df.columns for col in required_columns):
        logger.error(f"CSV-файл {csv_path} не содержит всех необходимых столбцов: {required_columns}")
        raise ValueError(f"CSV-файл {csv_path} не содержит всех необходимых столбцов")
    ...
    x_true = np.where(vis_pred[:, None], x_pred, 0).astype(np.float32)  # Истинные X = X для видимых, 0 для невидимых
    y_true = np.where(vis_pred[:, None], y_pred, 0).astype(np.float32)  # Истинные Y = Y для видимых, 0 для невидимых
    inpaint_mask = (1 - vis_pred).astype(np.float32)  # Маска: 1 для невидимых, 0 для видимых
    ...
    for i in range(0, len(frames) - seq_len + 1):
        if i + seq_len <= len(frames):
            coor_pred.append(np.stack([x_pred[i:i+seq_len], y_pred[i:i+seq_len]], axis=-1))  # [seq_len, 2]
            coor_true.append(np.stack([x_true[i:i+seq_len], y_true[i:i+seq_len]], axis=-1))  # [seq_len, 2]
    ...
    coor_pred[:, :, 0] /= 1920.0  # Нормализация X
    coor_pred[:, :, 1] /= 1080.0  # Нормализация Y
    coor_true[:, :, 0] /= 1920.0  # Нормализация X
    coor_true[:, :, 1] /= 1080.0  # Нормализация Y
...
def main():
    ...
    if args.gpu_memory_limit > 0:
        ...
        if gpus:
            try:
                ...
                logger.info(f"Установлено ограничение памяти GPU: {args.gpu_memory_limit} МБ")
            except RuntimeError as e:
                logger.error(f"Ошибка при установке ограничения памяти GPU: {e}")

    if args.seq < 1:
        logger.error("Длина последовательности должна быть не менее 1, получено %d", args.seq)
    ...
    if len(train_pairs) == 0:
        logger.error(
            "Не найдены тренировочные данные. Проверьте %s/train и файлы CSV.", DATASET_DIR
        )
        raise ValueError(
            "Не найдены тренировочные данные. Проверьте %s/train и файлы CSV." % DATASET_DIR
        )
    ...
    train_dataset = (
        tf.data.Dataset.from_tensor_slices(
            (
                [p[0] for p in train_pairs],  # video_path (None)
                [p[1] for p in train_pairs],  # csv_path
                [p[2] for p in train_pairs],  # frame_dir (None)
            )
        )
        .map(
            lambda t, c, f: tf.py_function(
                func=lambda x, y, z: load_coordinate_data(
                    x, y, z, "train", args.seq
                ),
                inp=[t, c, f],
    ...
    test_dataset = (
        tf.data.Dataset.from_tensor_slices(
            (
                [p[0] for p in test_pairs],  # video_path (None)
                [p[1] for p in test_pairs],  # csv_path
                [p[2] for p in test_pairs],  # frame_dir (None)
            )
        )
        .map(
            lambda t, c, f: tf.py_function(
